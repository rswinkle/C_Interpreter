...
test_array[9]=../tests/newtest_basic.txt
...
do
	echo "$f"
	valgrind --leak-check=full -v --show-reachable=yes --track-origins=yes ./cinterpreter $f 2>&1
	#echo "filename = $f"
done
...
do
	echo "$f"
	valgrind --leak-check=full -v --show-reachable=yes --track-origins=yes ./cinterpreter -E $f 2>&1
	#echo "filename = $f"
done
